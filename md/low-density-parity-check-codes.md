...
$$
\alpha_{i,j}(x) := K \sum_{c_{\ C(i)\setminus \{j\}}} \delta\left[\sum_{v\in C(i)\setminus \{j\}}c_v = x\right] \times \prod_{j'\in C(i)\setminus \{j\}} \beta_{i,j'}(c_{j'})
$$

ここで、定数$K$は$\alpha_{i,j}(0)+\alpha_{i,j}(1)=1$を満たすように定める。また、  

$$\sum_{c_{\{1, 2, \dots, n\}}} := \sum_{c_{1}\in {\mathbb F}_2}\sum_{c_{2}\in {\mathbb F}_2}\cdots \sum_{c_{n}\in {\mathbb F}_2}$$
...
$$
\beta_{i,j}(x) := K'P(y_j|x)\prod_{i'\in V(j)\setminus \{i\}}\alpha_{i',j}(x).
$$
...
$$
\gamma_{j}(x) := P(y_j|x)\prod_{i\in V(j)}\alpha_{i,j}(x).
$$
